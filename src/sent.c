...
static void (*handler[LASTEvent])(XEvent *) = {
    [ButtonPress] = bpress,        [ClientMessage] = cmessage,
    [ConfigureNotify] = configure, [Expose] = expose,
    [KeyPress] = kpress,
};
...
void ffdraw(Image *img) {
  ...
  XPutImage(xw.dpy, xw.win, d->gc, img->ximg, 0, 0, xoffset, yoffset,
  ...
  if (!im) {
    ...
    for (i = 0; i < slides[idx].linecount; i++)
      drw_text(d, (xw.w - width) / 2,
               (xw.h - height) / 2 + i * linespacing * d->fonts->h, width,
               d->fonts->h, 0, slides[idx].lines[i], 0);
...
void xinit() {
  ...
  xw.win = XCreateWindow(xw.dpy, XRootWindow(xw.dpy, xw.scr), 0, 0, xw.w, xw.h,
                         0, XDefaultDepth(xw.dpy, xw.scr), InputOutput, xw.vis,
...
void xloadfonts() {
  ...
  for (i = 0; i < NUMFONTSCALES; i++) {
    for (j = 0; j < LEN(fontfallbacks); j++) {
      if (MAXFONTSTRLEN < snprintf(fstrs[j], MAXFONTSTRLEN, "%s:size=%d",
                                   fontfallbacks[j], FONTSZ(i)))
...
void usage() { die("usage: %s [file]", argv0); }
